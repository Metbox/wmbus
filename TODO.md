...
Everything below is what's left to reach full upstream parity and complete
the port (Phases 6 tail → 7 → 8).
...
## 4. Phase 7 — Metbox integration

Once Phase 6 fixture coverage is acceptable (suggested threshold: 90%+ on
seed-driver fixtures, or 100% for drivers the Metbox seed actually uses),
perform the cutover:

- [ ] Replace [api/src/shared/wmbusmeters/index.ts](api/src/shared/wmbusmeters/index.ts)
      with `export * from "@metbox/wmbus";`
- [ ] Add `"@metbox/wmbus": "file:../wmbus"` to [api/package.json](api/package.json)
- [ ] Remove `build:wasm` script from api/package.json
- [ ] Run `npm test` in api/ — all inbound/rest-api integration tests pass
- [ ] Run Metbox dev server end-to-end against a known-good gateway
- [ ] Verify [process-wmbus.ts](api/src/services/inbound/process-wmbus.ts),
      [post-decode.ts](api/src/rest-api/routes/partner/wmbus/post-decode.ts),
      [get-drivers.ts](api/src/rest-api/routes/partner/wmbus/get-drivers.ts)
      still work unchanged (they just `import` from `@/shared/wmbusmeters`)
- [ ] Coverage-gate test: every `wmbusDriver` in
      [api/src/db/seed/device-models.ts](api/src/db/seed/device-models.ts)
      resolves through `listWmbusDrivers()`

## 5. Phase 8 — Shadow-mode burn-in

- [ ] Feature flag `WMBUS_PARITY_CHECK=1` in the inbound worker
- [ ] Decode every incoming telegram through both the TS port and the still-
      installed WASM, log mismatches with `log.warn`
- [ ] Dashboard: mismatch count per driver over 48h staging + prod traffic
- [ ] Fix any real-world telegram discrepancies the fixture suite missed
- [ ] After 48h of zero mismatches: delete [api/wmbusmeters-wasm/](api/wmbusmeters-wasm/),
      strip Emscripten from Dockerfile / CI
- [ ] Publish `@metbox/wmbus` to npm (or internal registry)
- [ ] Release notes: list any drivers intentionally dropped / driver coverage delta

## 6. Nice-to-have polish (can defer indefinitely)

- Publish on npm as `@metbox/wmbus` with proper CHANGELOG
- Add `analyze()` plain-text output format if any Metbox admin UI path relies on
  more than the JSON (currently we stringify the JSON — fine for Metbox)
